...
// Add domains which should be included in the certificate
$csrFields->setSans(
    [
        'ssl-test.de',
        'beispiel.ssl-test.de',
    ]
);
...
// Add add validation entries for every domain of the certificate
$certificateData->setValidation(
    [
        new Validation('ssl-test.de', Validation::METHOD_EMAIL, 'webmaster@ssl-test.de'),
        new Validation('beispiel.ssl-test.de', Validation::METHOD_EMAIL, 'webmaster@ssl-test.de'),
    ]
);

//Load a product: SECTIGO Lite Multidomain (not a trial certificate)
$productRepository = new ProductRepository($client);
$product = $productRepository->load('A000346');
